...
def test_high_risk_requires_active_approval_and_expired_approval_denies():
    authorizer = CapabilityAuthorizer((allow("agent-a", "repo.read", "repo/*"),))
    high = request(risk=RiskLevel.HIGH)
    assert authorizer.authorize(high).status.value == "approval_required"
    expired = Approval("operator", "approved", expires_at=datetime.now(UTC) - timedelta(seconds=1))
    assert authorizer.authorize(AuthorizationRequest(**{**high.__dict__, "approval": expired})).status.value == "approval_required"
    active = Approval("operator", "approved", expires_at=datetime.now(UTC) + timedelta(minutes=5))
    assert authorizer.authorize(AuthorizationRequest(**{**high.__dict__, "approval": active})).allowed


def test_external_egress_requires_explicit_policy():
    authorizer = CapabilityAuthorizer((allow("agent-a", "net.fetch", "net/*"),))
    ...
    assert authorizer.__class__((allow("agent-a", "net.fetch", "net/*"),), (policy,)).authorize(
