...
def test_regression_release_gate_and_deterministic_ab() -> None:
    ...
    report = engine.report(
        ...
        thresholds={"quality": 0.01},
    )
    assert report.case_count == 1
    assert report.evidence_digest
    ok, reasons = engine.release_gate(
        report,
        ReleaseGatePolicy(minimum_score=0.9, max_regressions=0),
    )
    assert ok, reasons
    bad = engine.regressions(
        {"quality": 0.7},
        {"quality": 0.9},
        {"quality": 0.05},
    )
    ...
    a = engine.assign_variant(spec, subject="subject-1")
    b = engine.assign_variant(spec, subject="subject-1")
    assert a == b
