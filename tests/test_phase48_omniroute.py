"""Phase 48 OmniRoute adapter tests."""
...
from core.runtime import InvocationRequest, InvocationStatus, OmniRouteBridge, OmniRouteCredentialRef, OmniRouteModel, OmniRoutePolicy, RuntimeErrorCode
...
def request(**metadata):
    return InvocationRequest(capability_id="model.infer", input="hello", project_id="demo", metadata=tuple(metadata.items()))
...
def test_remote_endpoint_requires_explicit_opt_in():
    bridge = OmniRouteBridge("https://example.com:20128", allow_remote=True, transport=FakeTransport())
...
def test_health_and_catalog_are_discoverable():
    transport = FakeTransport({
        ...
        ("GET", "/v1/models"): {"data": [
            {"id": "fast-model", "provider": "p1", "capabilities": ["text", "code"], "input_cost": 0.2},
            {"id": "vision-model", "provider": "p2", "capabilities": ["text", "vision"], "input_cost": 0.5},
        ]},
    })
...
def test_capability_aware_preferred_and_fallback_selection():
    ...
    models = (
        ...
        OmniRouteModel("code", capabilities=frozenset({"text", "code"}), input_cost=0.3),
        OmniRouteModel("expensive-code", capabilities=frozenset({"text", "code"}), input_cost=0.8),
    )
    policy = OmniRoutePolicy(required_capabilities=frozenset({"code"}), preferred_models=("missing",), fallback_models=("code",))
    assert bridge.select_model(policy, models=models).model_id == "code"
    assert bridge.select_model(OmniRoutePolicy(required_capabilities=frozenset({"code"})), models=models).model_id == "code"
...
def test_invocation_uses_openai_compatible_contract_and_usage():
    transport = FakeTransport({
        ("GET", "/v1/models"): {"data": [{"id": "router/model", "provider": "router", "capabilities": ["text"]}]},
        ("POST", "/v1/chat/completions"): {"choices": [{"message": {"content": "world"}}], "usage": {"prompt_tokens": 3, "completion_tokens": 2, "total_tokens": 5}},
    })
    bridge = OmniRouteBridge(transport=transport)
...
def test_structured_messages_are_forwarded_without_governance_metadata():
    transport = FakeTransport({
        ...
        ("POST", "/v1/chat/completions"): {"choices": [{"message": {"content": "ok"}}]},
    })
    bridge = OmniRouteBridge(transport=transport)
    req = InvocationRequest(capability_id="model.infer", project_id="p", input={"messages": [{"role": "user", "content": "x"}]}, metadata=(("preferred_models", "m"), ("authority_scope", "admin")))
...
def test_credential_is_resolved_at_request_time_and_not_stored_as_value(monkeypatch):
    ...
    bridge = OmniRouteBridge(transport=transport, credential_ref=OmniRouteCredentialRef("prod", "SI_OMNI_TEST_TOKEN"))
    bridge.health()
    assert transport.calls[-1][-1] == "secret-token"
    assert "secret-token" not in repr(bridge.credential_ref)


def test_missing_credential_does_not_fail_discovery():
    os.environ.pop("OMNIROUTE_TOKEN", None)
    transport = FakeTransport({("GET", "/health"): {"healthy": True}})
    assert OmniRouteBridge(transport=transport).health()["healthy"] is True


def test_rate_limit_and_server_errors_are_retryable_without_body_leakage():
    from core.runtime.omniroute import OmniRouteTransportError

    transport = FakeTransport(errors={("GET", "/v1/models"): OmniRouteTransportError(429, "http_error", 7.0)})
    bridge = OmniRouteBridge(transport=transport)
...
def test_network_failure_maps_to_timeout_and_is_retryable():
    from core.runtime.omniroute import OmniRouteTransportError

    transport = FakeTransport(errors={("GET", "/v1/models"): OmniRouteTransportError(None, "transport_error")})
...
def test_no_matching_model_is_typed_failure():
    transport = FakeTransport({("GET", "/v1/models"): {"data": [{"id": "text", "capabilities": ["text"]}]}})
    response = OmniRouteBridge(transport=transport).invoke(request(required_capabilities="vision"))
    assert response.status is InvocationStatus.FAILED
    assert response.error is not None
