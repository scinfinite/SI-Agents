...
class OrganizationExpansionTests(unittest.TestCase):
    ...
    def test_every_division_has_exactly_one_home_team(self) -> None:
        ...
        self.assertEqual(set(assignments), {division.id for division in load_catalog(CATALOG).all_divisions()})
    ...
    def test_workflow_agents_must_belong_to_the_declared_team(self) -> None:
        ...
        errors = candidate.validate(load_catalog(CATALOG).all_divisions(), load_catalog(CATALOG).all())
