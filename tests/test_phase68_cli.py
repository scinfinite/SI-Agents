...
def test_trailing_json_option_is_supported_by_phase68_dispatch(capsys) -> None:
    from core.cli.dispatch import main as dispatch_main
    assert dispatch_main(["task", "list", "--json"]) == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["command"] == "run.list"


def test_legacy_run_is_not_reinterpreted_as_phase68_run(monkeypatch) -> None:
    legacy = importlib.import_module("core.cli.main")
