...
def _request(server, method, path, body=None, subject="u1", project="p1"):
    ...
    request = urllib.request.Request(
        ...
        data=payload,
        method=method,
        headers={"Content-Type": "application/json", "X-SI-Subject": subject, "X-SI-Project": project},
    )
...
def test_wait_control_api_round_trip_and_isolation(tmp_path: Path):
    ...
    try:
        ...
        status, created = _request(server, "POST", "/api/v1/waits", {"kind": "timer", "wake_at": wake_at, "payload": {"usage": {"input_tokens": 2}}})
        assert status == 201
