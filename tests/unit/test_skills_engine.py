...
def make_skill(**overrides: object) -> Skill:
    values: dict[str, object] = {
        ...
        "procedure": ("perform the operation",),
        "inputs": ("value",),
        "outputs": ("result",),
        "prerequisites": ("precondition",),
        "required_tools": ("test-runner",),
        "requested_capabilities": (),
        "requested_permissions": ("repository_read",),
        "verification": ("result is correct",),
        "failure_behavior": ("return failed verification",),
        "evidence_requirements": ("record result",),
        "examples": ("test example",),
        "compatibility": (SkillRequirement("si-core", "runtime", "1.x"),),
        "provenance": "SI-native test provenance",
...
def test_executor_denies_ungranted_capability() -> None:
    registry = SkillRegistry()
    skill = registry.register(make_skill(requested_capabilities=("workspace_write",)))
...
def test_executor_fails_closed_on_unverified_result() -> None:
    registry = SkillRegistry()
    skill = registry.register(make_skill())
    result = SkillExecutor(registry, PermissionEngine(), EvidenceStore()).execute(
        skill.id,
        lambda _: {"result": "bad"},
        {"value": 1},
...
def test_blocked_skill_cannot_execute() -> None:
    registry = SkillRegistry()
    skill = registry.register(make_skill(status=SkillStatus.BLOCKED))
    with pytest.raises(SkillExecutionError, match="not executable"):
        SkillExecutor(registry, PermissionEngine(), EvidenceStore()).execute(
            skill.id,
            lambda _: {"result": "ok"},
            {"value": 1},
