...
def _reject_secret_keys(value: Any, path: str = "payload") -> None:
    if isinstance(value, Mapping):
        secret_tokens = ("password", "passwd", "secret", "api_key", "private_key", "credential", "access_token", "auth_token", "refresh_token", "bearer")
...
class HumanApprovalService:
    ...
    def create(self, payload: Mapping[str, Any]) -> ApprovalRequest:
        ...
        kind, gate, action, summary = (_text(payload.get(key), key) for key in ("kind", "gate", "action", "summary"))
        if kind not in _ALLOWED_KINDS or gate not in _ALLOWED_GATES:
            raise ValueError("unsupported approval kind or gate")
        try:
            estimated_cost, expires_in = float(payload.get("estimated_cost", 0.0)), float(payload.get("expires_in", 3600.0))
        except (TypeError, ValueError) as exc:
        ...
        encoded_payload = _bounded_json(decision_payload)
        now = time.time()
        approval_id = f"approval_{uuid.uuid4().hex}"
        fingerprint = _bounded_json({"approval_id": approval_id, "subject_id": subject_id, "project_id": project_id, "action": action, "gate": gate, "risk": payload.get("risk", "low"), "estimated_cost": estimated_cost, "external_egress": bool(payload.get("external_egress", False)), "destructive": bool(payload.get("destructive", False)), "deployment": bool(payload.get("deployment", False))})
        ...
        with self._lock:
            ...
            values = (approval_id, subject_id, project_id, kind, gate, action, summary, requested_by, now, now + expires_in, ApprovalState.PENDING.value, str(payload.get("risk", "low")), estimated_cost, int(bool(payload.get("external_egress", False))), int(bool(payload.get("destructive", False))), int(bool(payload.get("security_sensitive", False))), int(bool(payload.get("deployment", False))), payload.get("session_id"), payload.get("run_id"), 0, None, None, None, encoded_payload, digest)
    ...
    def decide(self, approval_id: str, *, subject_id: str, project_id: str, decision: ApprovalState, decision_by: str, reason: str, payload: Mapping[str, Any] | None = None, expected_revision: int | None = None) -> ApprovalDecision:
        ...
        _reject_secret_keys(decision_payload, "decision")
        encoded = _bounded_json(decision_payload)
        with self._lock:
