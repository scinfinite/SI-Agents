...
def _sanitize(value: object, depth: int = 0) -> object:
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            ...
            output[key] = "[REDACTED]" if normalized in _SECRET_KEYS else _sanitize(
                item, depth + 1
            )
        return output
    ...
    if isinstance(value, str):
        ...
        return "[REDACTED]" if any(pattern in lowered for pattern in patterns) else value
...
class TelemetryStore:
    def __init__(self, path: str | Path = ":memory:") -> None:
        ...
        self.db.execute(
            "CREATE INDEX IF NOT EXISTS ix_tel_time ON telemetry(occurred_at,record_id)"
        )
        self.db.execute(
            "CREATE INDEX IF NOT EXISTS ix_tel_scope "
            "ON telemetry(tenant_id,project_id,occurred_at)"
        )
    ...
    def get(self, record_id: str) -> TelemetryRecord:
        row = self.db.execute(
            "SELECT * FROM telemetry WHERE record_id=?", (record_id,)
        ).fetchone()
        if row is None:
    ...
    def query(
        ...
        *,
        ...
        sql = (
            ...
            + " ORDER BY occurred_at,record_id LIMIT ? OFFSET ?"
        )
        rows = self.db.execute(
            sql, (*params, bounded_limit, bounded_offset)
        ).fetchall()
    ...
    def integrity(
        ...
        *,
        ...
        for record in self.query(
            ...
            if hashlib.sha256(_canon(envelope).encode()).hexdigest() != record.digest:
                return False
        return True
