...
class ImprovementStore:
    ...
    def append(self, kind: str, record: object) -> str:
        ...
        payload = json.dumps(record, default=str, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
        previous = self.db.execute("SELECT digest FROM improvement_history ORDER BY sequence DESC LIMIT 1").fetchone()
    ...
    def verify(self) -> bool:
        previous = "0" * 64
        rows = self.db.execute("SELECT kind,payload_json,previous_digest,digest FROM improvement_history ORDER BY sequence").fetchall()
        for row in rows:
            if row["previous_digest"] != previous:
                return False
            expected = hashlib.sha256(f"{previous}|{row['kind']}|{row['payload_json']}".encode()).hexdigest()
...
class ContinuousImprovement:
    ...
    def analyze(
        ...
        *,
        ...
        current = dict(dimensions or {
            ...
            "latency": 1.0 / (1.0 + sum(result.duration_ms for result in run.results) / len(run.results) / 1000),
        })
    ...
    def canary_allowed(
        ...
        *, samples: int, error_rate: float, cost_ratio: float, latency_ratio: float, quality: float,
        ...
        if cost_ratio > policy.maximum_cost_ratio: reasons.append("canary cost ratio exceeded")
        if latency_ratio > policy.maximum_latency_ratio: reasons.append("canary latency ratio exceeded")
        if quality < policy.minimum_quality: reasons.append("canary quality floor not met")
        return not reasons, tuple(reasons)

    @staticmethod
    def require_approval(recommendation: OptimizationRecommendation, approval: ApprovalRecord | None) -> bool:
        consequential = recommendation.risk in {ChangeRisk.HIGH, ChangeRisk.CRITICAL} or recommendation.target in {ImprovementTarget.ROUTING, ImprovementTarget.MODEL, ImprovementTarget.WORKFLOW}
        return consequential and not (approval and approval.approved and approval.recommendation_id == recommendation.recommendation_id)

    @staticmethod
    def rollback_plan(recommendation: OptimizationRecommendation, previous_version: str, trigger: str) -> RollbackPlan:
        if recommendation.status not in {RecommendationStatus.CANARY, RecommendationStatus.PROMOTED}:
            raise ImprovementError("rollback requires an active canary or promoted recommendation")
        return RollbackPlan(f"rollback_{uuid.uuid4().hex}", recommendation.recommendation_id, previous_version, trigger)
