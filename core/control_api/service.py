...
class ControlApiService:
    ...
    Human approvals are likewise state/governance records: a decision never grants
    capabilities or credentials by itself; downstream execution must re-authorize.
    """
    ...
    def snapshot(self) -> ApiSnapshot:
        with self._lock:
            counts = (("agents", len(self._agents.all())), ("teams", len(self._teams.all())), ("workflows", len(self._organization.workflows)), ("runs", len(self._runs)), ("events", len(self._events)))
            return ApiSnapshot(API_VERSION, "SI-Agents Control API", counts)

    def agents(self) -> list[dict[str, object]]:
        return [{"id": a.id, "name": a.name, "division": a.division, "status": a.status.value} for a in sorted(self._agents.all(), key=lambda item: (item.division, item.name, item.id))]

    def teams(self) -> list[dict[str, object]]:
        return [{"id": t.id, "name": t.name, "description": t.description, "tasks": [x.id for x in t.tasks]} for t in sorted(self._teams.all(), key=lambda item: item.id)]

    def organization(self) -> dict[str, object]:
        return {"teams": [{"id": t.id, "name": t.name, "purpose": t.purpose, "division_ids": list(t.division_ids), "member_agent_ids": list(t.member_agent_ids), "lead_agent_id": t.lead_agent_id} for t in self._organization.teams], "division_assignments": [{"division_id": a.division_id, "team_id": a.team_id, "rationale": a.rationale} for a in self._organization.division_assignments], "workflows": [{"id": w.id, "name": w.name, "purpose": w.purpose, "steps": [{"id": s.id, "kind": s.kind.value, "team_id": s.team_id, "agent_id": s.agent_id, "purpose": s.purpose, "depends_on": list(s.depends_on)} for s in w.steps]} for w in self._organization.workflows]}
    ...
    def skills(self) -> list[dict[str, object]]:
        ...
        if not skills_root.exists():
            return []
        return [{"id": path.parent.name, "path": str(path.relative_to(self.root))} for path in sorted(skills_root.rglob("SKILL.md"))]
    ...
    def governance_state(self) -> dict[str, object]:
        ...
        return {"permissions": [{"subject": x.subject, "capability": x.capability, "scope": x.scope, "effect": x.effect.value, "conditions": list(x.conditions)} for x in snapshot.permissions], "policies": [{"name": x.name, "description": x.description, "deny_capabilities": list(x.deny_capabilities), "approval_risks": [r.value for r in x.approval_risks], "max_cost": x.max_cost, "allow_external_egress": x.allow_external_egress} for x in snapshot.policies], "capabilities": [{"name": x.name, "description": x.description, "risk": x.risk.value} for x in snapshot.capabilities]}

    def evidence(self) -> dict[str, object]:
        with self._lock:
            return {"events": [event.as_dict() for event in self._events], "run_count": len(self._runs), "queued_runs": sum(run.status is RunStatus.QUEUED for run in self._runs.values()), "note": "Execution evidence is produced by downstream execution and verification components; this view reports only control-plane evidence."}
    ...
    def visualization(self) -> dict[str, object]:
        ...
        for agent in agents:
            ...
            for skill in agent.skills:
                skill_id = f"skill:{skill}"; add_node(skill_id, "skill", skill); add_edge(f"agent:{agent.id}", skill_id, "uses")
            for capability in agent.capabilities:
                cap_id = f"capability:{capability}"; add_node(cap_id, "capability", capability); add_edge(f"agent:{agent.id}", cap_id, "requests")
            for permission in agent.permissions:
                perm_id = f"permission:{agent.id}:{permission}"; add_node(perm_id, "permission", permission, subject=agent.id); add_edge(f"agent:{agent.id}", perm_id, "declares")
        for permission in self.governance.store.snapshot().permissions:
            cap_id = f"capability:{permission.capability}"; add_node(cap_id, "capability", permission.capability); subject_id = f"agent:{permission.subject}"
        ...
        for workflow in self._organization.workflows:
            workflow_id = f"workflow:{workflow.id}"; add_node(workflow_id, "workflow", workflow.name, purpose=workflow.purpose)
            for step in workflow.steps:
                step_id = f"step:{workflow.id}:{step.id}"; add_node(step_id, "step", step.id, workflow=workflow.id, step_kind=step.kind.value, purpose=step.purpose); add_edge(workflow_id, step_id, "contains")
                target = f"agent:{step.agent_id}" if step.agent_id else f"team:{step.team_id}"
                if target in nodes: add_edge(step_id, target, "assigned")
                for dependency in step.depends_on: add_edge(f"step:{workflow.id}:{dependency}", step_id, "depends_on")
        with self._lock:
    ...
    def decide_approval(self, approval_id: str, payload: dict[str, object]) -> dict[str, object]:
        subject_id, project_id = payload.get("subject_id"), payload.get("project_id")
        ...
        result = self.approvals.decide(approval_id, subject_id=subject_id, project_id=project_id, decision=state, decision_by=str(payload.get("decision_by", "")), reason=str(payload.get("reason", "")), payload=payload.get("result", {}) if isinstance(payload.get("result", {}), dict) else {}, expected_revision=payload.get("expected_revision") if isinstance(payload.get("expected_revision"), int) else None)
        with self._lock:
    ...
    def create_run(self, payload: dict[str, Any]) -> dict[str, object]:
        action = payload.get("action"); subject = payload.get("subject")
        ...
        capabilities = payload.get("capabilities", []); provenance = payload.get("provenance", [])
        if not isinstance(capabilities, list) or not all(isinstance(x, str) for x in capabilities): raise ValueError("capabilities must be a list of strings")
        if not isinstance(provenance, list) or not all(isinstance(x, str) for x in provenance): raise ValueError("provenance must be a list of strings")
        try:
            risk = RiskLevel(payload.get("risk", RiskLevel.LOW.value)); data_class = DataClass(payload.get("data_class", DataClass.PUBLIC.value)); estimated_cost = float(payload.get("estimated_cost", 0.0))
        except (ValueError, TypeError) as exc: raise ValueError("invalid risk, data_class, or estimated_cost") from exc
        if estimated_cost < 0: raise ValueError("estimated_cost must be non-negative")
        request = GovernanceRequest(action=action, risk=risk, data_class=data_class, external_egress=bool(payload.get("external_egress", False)), paid_resource=bool(payload.get("paid_resource", False)), destructive=bool(payload.get("destructive", False)), production=bool(payload.get("production", False)), credential=bool(payload.get("credential", False)), publication=bool(payload.get("publication", False)), estimated_cost=estimated_cost, capabilities=tuple(capabilities), subject=subject, provenance=tuple(provenance))
        decision = self.governance.decide(request)
        if decision.status.value != "allow": raise PermissionError(json.dumps({"status": decision.status.value, "reasons": list(decision.reasons)}))
        ...
        with self._lock:
            self._runs[run.id] = run; self._events.append(event)
        return run.as_dict()
