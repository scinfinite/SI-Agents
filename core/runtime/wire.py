...
def _wire_safe(payload: dict[str, Any]) -> dict[str, Any]:
    try:
        json.dumps(payload, separators=(",", ":"), allow_nan=False)
    except (TypeError, ValueError) as exc:
        raise ValueError("runtime wire payload must contain JSON-compatible values") from exc
    return payload
...
def governance_to_dict(value: GovernanceRequest) -> dict[str, Any]:
    ...
    return {
        ...
        "paid_resource": value.paid_resource,
    ...
    return _wire_safe(
        {
            ...
            "governance": governance_to_dict(value.governance) if value.governance else None,
