...
OpenCode documents a headless server started with `opencode serve`, with a default local listener at `127.0.0.1:4096`. Its HTTP API provides session creation, synchronous session messages, asynchronous prompts, session abort, and an SSE event stream. Phase 23 uses the synchronous message and abort endpoints because they map cleanly to the current SI runtime contract; streaming remains explicitly unsupported until event normalization is implemented.
